...
def parse_countries():
    ...
    soup = BeautifulSoup(response.text, "html.parser")  # BeautifulSoup() поместит текст ответа в переменную soup
    tables = soup.find_all("table", class_="wikitable")  # Выбираем только таблицы класса wikitable
    ...
    for table in [*tables[:2], *tables[3:]]:
        countries_soup = table.find_all("tr")[1:]  # Выбираем из таблиц <tr> все строки начиная с второго

        # Перебор циклом soup стран для получения заданных переменных
        for country_soup in countries_soup:
            country_name = country_soup.find("a").get("title")  # из soup страны найти("а").получить("название")
            full_country_name = country_soup.find_all("td")[-1].text.replace("\n", "")  # найти полное название страны
            flag_url = country_soup.find("a").find("img").get("src")  # найти("a") найти("img") получить("источник")
            words_in_full_country_name = len(full_country_name.split(" "))  # кол-во слов в полном названии страны
