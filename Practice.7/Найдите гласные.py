...
def vowel_indices(word):
    ...
    for index, letter in enumerate(word):  # добавить индекс с перечислением
        if letter in vowels:
            vowelsList.append(index + 1)  # добавить 1, так как список/массивы начинаются с 0
    return vowelsList
...
test3 = assert_equals(vowel_indices("UNDERARMED"), [1, 4, 6, 9], "failed on the word 'UNDERARMED'. Consider case")
