...
def update(bg_color, screen, stats, scor, gun, inos, bullets):
    """обновление экрана"""
...
def update_bullets(screen, stats, scor, inos, bullets):
    ...
    if collisions:
        for inos in collisions.values():
            stats.score += len(inos) * 10
        scor.image_score()
        check_high_score(stats, scor)
        scor.image_guns()
    if len(inos) == 0:
        bullets.empty()
...
def create_army(screen, inos):
    ...
    number_ino_y = int((700 - 300 - 2 * ino_height) / ino_height)
...
def check_high_score(stats, scor):
    """Проверка новых рекордов"""
    if stats.score > stats.high_score:
        stats.high_score = stats.score
        scor.image_high_score()
