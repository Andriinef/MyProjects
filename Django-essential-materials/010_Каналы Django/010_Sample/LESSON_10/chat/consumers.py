...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def chat_message(self, event):
        ...
        await self.send(
            text_data=json.dumps({"message": message, "date": str(date.today())})
        )

    @database_sync_to_async
    def new_message(self, message):
        Message.objects.create(
            text=message, user=self.user, room=Room.objects.get(id=self.room_name)
        )
    ...
    def get_historical_data(self):
        return dict(
            Message.objects.filter(
                user=self.user, room=Room.objects.get(id=self.room_name)
            ).values_list("timestamp", "text")
        )
