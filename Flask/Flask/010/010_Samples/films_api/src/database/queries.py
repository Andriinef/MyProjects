...
harry_potter_and_ch_s = (
    db.session.query(models.Film)
    .filter(models.Film.title == "Harry Potter and Chamber of Secrets")
    .first()
)
harry_potter_priz_az = (
    db.session.query(models.Film)
    .filter_by(title="Harry Potter and the Prizoner of Azkaban")
    .first()
)
and_statement_harry_potter = (
    db.session.query(models.Film)
    .filter(
        and_(
...
deathy_hallows = (
    db.session.query(models.Film)
    .filter(models.Film.title.ilike("%deathly hallows%"))
    .all()
)
