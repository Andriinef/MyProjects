...
class LoginSerializerTestCase(TestCase):
    ...
    def test_validate_with_wrong_credentials(self):
        credentials = {"username": "test1", "password": "pass"}
        serializer = LoginSerializer(data=credentials)
        self.assertFalse(serializer.is_valid())
        self.assertGreater(len(serializer.errors), 0)
        self.assertRegex(
            serializer.errors["non_field_errors"][0], "Incorrect username\/password"
        )
    ...
    def test_validate_without_password(self):
        ...
        self.assertRegex(serializer.errors["password"][0], "required")
    ...
    def test_validate_without_username(self):
        ...
        self.assertRegex(serializer.errors["username"][0], "required")
