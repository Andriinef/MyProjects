...
def get_data(file_path):
    ...
    for url in urls_list:
        ...
        item_phones_list = []
        try:
            item_phones = soup.find("div", class_="service-phones-list").find_all(
        ...
        try:
            item_site = (
                soup.find(text=re.compile("Сайт|Официальный сайт"))
                .find_next()
                .text.strip()
            )
        except Exception as _ex:
        ...
        try:
            item_social_networks = (
                soup.find(text=re.compile("Страница в соцсетях"))
                .find_next()
                .find_all("a")
            )
